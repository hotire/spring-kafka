...
이러한 특징 덕분에 Kafka는 기존 메시징 시스템(Active MQ, Rabbit MQ)을 대체하기도 하고 시스템 로그를 모으거나 데이터 가공을 위해 파이프라인을 구성할 때 등 다양한 경우에 사용합니다. 
...
Kafka 클라이언트의 상태는 총 4가지로 DISCONNECTED / CONNECTING / CHECKING_API_VERSIONS / READY 가 존재하는데요. 

READY 상태에서 오랫동안 아무일도 하지 않으면 연결을 닫고 처리하는 클래스입니다. 
...
Kafka 클라이언트에서 매번 하나씩 요청하고 기다리면 성능적으로 너무 느리잖아요. 

그러다보니 이전에 보낸 요청에 대한 응답을 받기 전에 다음 요청을 브로커로 전송할 수 있도록 

전송은 되었지만 응답을 아직 받지 못한 요청을 의미합니다. 

Deque 에 저장하여 관리한다. 

브로커로부터 응답이 도착하면 그 브로커에 보냈던 요청을 저장해둔 Deque의 가장 앞쪽에서 요청을 하나 제거하고 응답을 처리합니다. 

만약 요청에 대한 응답에 오류가 있다면 Deque의 가장 앞에 있던 요청을 가장 뒤로 다시 넣고 요청을 브로커에 재전송합니다. 

이렇게 때문에 max.in.flight.requests.per.connection 설정값이 1보다 크면 전송순서가 바뀔수 있습니다. 
...
클러스터 관련 파티션 추가 브로커 추가, 제거 등의 정보를 업데이트 하기 위한 방법 


다음의 경우에 메타데이터 갱신이 필요하다.
